...
use AntCloudSDKCore\Exception\ErrorCodes;
...
class AntCloudClient
{
    ...
    const ANTCLOUD_SDK_VERSION = 'PHP-SDK-1.0.0';
    ...
    public function execute($request, $returnRawResp = false)
    {
        ...
        if ($respDecoded === null || !array_key_exists('response', $respDecoded) || $respDecoded['response'] === null) {
            throw new ClientException(ErrorCodes::ANTCLOUD_SDK_TRANSPORT_ERROR, "Unexpected gateway response: $httpResponse->body");
        }
        ...
        if (CommonUtil::isSuccessResp($respContent) && $this->checkSign) {
            ...
            if (!array_key_exists('sign', $respDecoded)) {
                throw new ClientException(ErrorCodes::ANTCLOUD_SDK_BAD_SIGNATURE, 'Invalid signature in response');
            }
